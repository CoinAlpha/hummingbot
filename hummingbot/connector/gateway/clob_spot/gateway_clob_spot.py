...
class GatewayCLOBSPOT(ExchangePyBase):
    ...
    async def _place_batch_order_create(self, orders_to_create: List[InFlightOrder]) -> List[PlaceOrderResult]:
        batch_update_result = await self._api_data_source.batch_order_update(
            orders_to_create=orders_to_create, orders_to_cancel=[]
        )
        return batch_update_result.place_order_results

    async def _place_batch_order_cancel_discretely(self, orders_to_cancel: List[InFlightOrder]) -> List[CancelOrderResult]:
        batch_update_result = await self._api_data_source.batch_order_update(
            orders_to_create=[], orders_to_cancel=orders_to_cancel
        )
        return batch_update_result.cancel_order_results
